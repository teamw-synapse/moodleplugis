...
class course_user_state_store implements course_state_store,
    ...
    public function increase($id, $amount) {
        ...
        if ($record = $this->exists($id)) {
            ...
            //storing overall table data for each user
            if($this->table == 'block_xp'){
                $subrecord = new stdClass();
                $xplevel = $this->db->get_record('block_xp_config',array('courseid' => 1));
                $levlsdata = json_decode($xplevel->levelsdata);
                if($dataexist = $this->db->get_record('block_levelup',array('userid' => $id))){
                    $subrecord->id = $dataexist->id;
                    $subrecord->userid = $id;
                    $subrecord->xp_points = intval($dataexist->xp_points)+intval($amount);
                    if ($returval = $this->isValueBetweendata($subrecord->xp_points, $levlsdata->xp)) {
                        $subrecord->xp_levels = $returval['level'];
                    }else{
                        $subrecord->xp_levels = 1;
                    }

                    $subrecord->timemodified = time();
                    $this->db->update_record('block_levelup', $subrecord);

                    $xpranks = $this->db->get_records_sql('SELECT s1.id, 
                           s1.userid, s1.xp_points, s1.xp_levels,  
                           (SELECT COUNT(DISTINCT s2.xp_points) 
                             FROM mdl_block_levelup AS s2 
                             WHERE s2.xp_points > s1.xp_points) + 1 AS xp_rank
                            FROM mdl_block_levelup AS s1
                            ORDER BY xp_rank');
                    $data = array();
                    foreach ($xpranks as $xprank) {
                        $this->db->update_record('block_levelup', $xprank);
                    }

                }else{
                    $subrecord->userid = $id;
                    $subrecord->xp_points = intval($amount);
                    ...
                    $subrecord->timemodified = time();
                    $this->db->insert_record('block_levelup', $subrecord);
    ...
    public function isValueBetweendata($value, $array) {
    ...
    protected function insert($id, $amount) {
        ...
        if($this->table == 'block_xp'){
            ...
            if($dataexist = $this->db->get_record('block_levelup',array('userid' => $id))){
                $subrecord->id = $dataexist->id;
                $subrecord->userid = $id;
                $subrecord->xp_points = $dataexist->xp_points+$amount;
                if ($returval = $this->isValueBetweendata($subrecord->xp_points, $levlsdata->xp)) {
                    $subrecord->xp_levels = $returval['level'];
                }else{
                    $subrecord->xp_levels = 1;
